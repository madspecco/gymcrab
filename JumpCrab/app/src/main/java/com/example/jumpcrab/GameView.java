...
public class GameView extends SurfaceView implements SurfaceHolder.Callback, SensorEventListener {
    ...
    public GameView(Context context, AttributeSet attrs) {
        ...
        mHandler = new Handler() {
            ...
            public void handleMessage(Message m) {
                ...
                } else if(m.getData().getBoolean("lose")) {
                    endGame();
                } else {
                    //So it is a status
    ...
    public void setLevel(Level level) { this.mLevel = level; }
    ...
    /**Set listener for when game ends
     * @param toAdd GameEndListener to notify when game ends*/
    public void setOnGameEndListener(GameEndListener toAdd) {
        gameEndListener = toAdd;
    }

    /**Notify listener that the game has ended*/
    private void endGame() {
        gameEndListener.gameHasEnded();
    }
        ...
        if(thread != null) {
            ...
            if(thread.getState() == Thread.State.NEW){
                ...
                if(thread.getState() == Thread.State.TERMINATED){
                    ...
                    thread = new TheGame(this, mLevel);
                    thread.setRunning(true);
                    thread.start();
                }
            }
    ...
    /** What to do when accuracy changes*/
    @Override
